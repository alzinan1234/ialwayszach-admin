    ...
    <>
      <aside
        className={`fixed top-0 left-0 h-full text-white z-50 transform transition-transform duration-300 ease-in-out 
          border-r-[0.5px] border-[#FFFFFF80] 
          bg-[linear-gradient(171deg,_#525252_1.81%,_#17191A_15.25%)] 
          shadow-[0_4px_24.1px_0_rgba(0,0,0,0.25)] 
          ${isOpen ? 'translate-x-0 w-64' : '-translate-x-full w-64'}`
        }
      >
        <div className="flex flex-col h-full justify-between">
          {/* Logo & Close Button */}
          <div className="flex items-center justify-between border-b border-[#FFFFFF80] pb-4 p-[26px]">
            <Image className='' src="/side-bar.png" alt="logo" width={55} height={36} />
          ...
          <nav className="mt-4 space-y-6 flex-grow overflow-y-auto">
            {navItems.map(({ name, href, icon: Icon }) => {
              ...
              return (
                <Link
                  key={name}
                  href={href}
                  className={`flex items-center px-4 w-[218px] mx-auto py-2 transition-all rounded-[4px] ${
                    isActive
                      ? 'text-[#9BD71B] border-[0.5px] border-[rgba(232,255,183,0.29)] bg-[linear-gradient(270deg,_#282828_0%,_rgba(155,215,27,0.10)_100%)]'
                      : 'hover:bg-zinc-800'
                  }`}
                >
            ...
            {/* Logout Button */}
            <div className="border-t border-[#FFFFFF80] pt-6 ">
